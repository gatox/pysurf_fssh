...
class VelocityVerletPropagator:
    ...
    def __init__(self, state):
        ...
        if self.state.method == "Surface_Hopping":
            ...
            raise SystemExit(f"Method '{self.state.method}' is not known!")
    ...
    def xi_half(self, state, dt):
        return state.xi + 0.5 * state.q_eff * dt * (
            state.ekin - 0.5 * (3 * state.natoms + 1) * state.t_target
        )
    ...
    def xi_t_dt(self, state, a_0, dt):
        ...
        return xi_05 + 0.5 * state.q_eff * dt * (
            ekin_05 - 0.5 * (3 * state.natoms + 1) * state.t_target
        )
...
class BornOppenheimer:
    ...
    def __init__(self, state):
        ...
        self.spp = SurfacePointProvider.from_questions(
            ["energy", "gradient"] + state.save_properties,
            self.nstates,
            self.natoms,
            config="spp.inp",
    ...
    def setup(self, state):
        result = self.compute(
            ["energy", "gradient"], state.crd, curr_state=state.instate
        )
    ...
    def new_surface(self, state, results, crd_new, t, dt):
        result = self.compute(["energy", "gradient"], crd_new, state.instate)
...
class Propagator:
    ...
    def grad(self, ene_cou_grad):
        ...
        if self.prob_name == "diagonal":
            ...
            g_diag = (
                {}
            )  # it must request all the gradients for all the electronic states in diagonal approach
    ...
    def mch_propagator_interpolator(
        self, ene_old, ene_new, vk_old, vk_new, substeps, dt
    ):
        ...
        for index in range(1, substeps + 1):
            p_i = dot(
                p_i,
                self._prop_inter_i(
                    ene_old, ene_new, vk_old, vk_new, index, substeps, dt
                ),
            )
        return p_i, h_total
    ...
    def hopping_probability(self, c_j_dt, c_i_dt, c_i_t, p_diag_ji, p_diag_ii):
        prob_factor_1 = 1 - abs(dot(c_i_dt, c_i_dt.conj())) / abs(
            dot(c_i_t, c_i_t.conj())
        )
        prob_factor_2_N = (dot(c_j_dt, dot(p_diag_ji.conj(), c_i_t.conj()))).real
        prob_factor_2_D = (
            abs(dot(c_i_t, c_i_t.conj()))
    ...
    def probabilities_tully(self, state, vk_old, ene_new, dt):
        ...
        if state.substeps:
            ...
            p_mch, h_total = self.mch_propagator_interpolator(
                ene_old, ene_new, vk_old, vk_new, substeps, dt
            )
        else:
            p_mch, h_total = self.mch_propagator(ene_old, ene_new, vk_old, vk_new, dt)
        probs = (2.0 * imag(rho_old[instate, :] * h_total[:, instate]) * dt) / (
            real(rho_old[instate, instate])
        )
        probs[instate] = 0.0
    ...
    def probabilities_diagonal(self, state, diag_prop):
        ...
        instate = state.instate
        for i in range(self.nstates):
            probs[i] = self.hopping_probability(
                c_diag_dt[i],
                c_diag_dt[instate],
                c_diag[instate],
                p_diag_dt[i, instate],
                p_diag_dt[instate, instate],
            )
        probs[instate] = 0.0
        return probs
    ...
    def lz_select(self, state, dt):
        ...
        if state.e_two_prev_steps is None:
            ...
            for istate in range(self.nstates):
                ...
                # compute probability
                if self.abs_gt(d_curr, d_prev_step) and self.abs_gt(
                    d_two_prev_steps, d_prev_step
                ):
                    curr_hop_prob = self.probabilities_lz(
                        d_curr, d_prev_step, d_two_prev_steps, dt
                    )
    ...
    def probabilities_lz(self, d_curr, d_prev_step, d_two_prev_steps, dt):
        ...
        return exp(
            (-pi / 2.0) * sqrt(abs(d_prev_step) ** 3 / abs(finite_difference_grad))
        )
    ...
    def energy_based_decoherence_correction(self, state, ncoeff):
        ...
        ncoeff_prima[curr] = ncoeff[curr] * sqrt(
            (1.0 - add) / (abs(dot(ncoeff[curr], ncoeff[curr].conj())))
        )
    ...
    def new_ncoeff(self, state, grad_probs, hop, att, succ):
        if self.prob_name == "diagonal":
            ...
            c_rho_new = self.elec_density_new(
                state, grad_probs.tully.rho_old, grad_probs.tully.p_mch
            )
...
class RescaleVelocity:
    ...
    def direction(self, vel, state_new):
        if self.rescale_vel == "nacs" == self.coupling:
            return (0.5) * (
                self.nac_old[state_new, self.state_old]
                + self.nac_new[state_new, self.state_old]
            )
        elif self.rescale_vel == "nacs" and self.coupling == "semi_coup":
...
class SurfaceHopping(BornOppenheimer):
    ...
    def __init__(self, state):
        ...
        if self.coupling == "nacs" and self.prob == "tully":
            needed_properties = ["energy", "gradient", "nacs"] + state.save_properties
            self.spp = SurfacePointProvider.from_questions(
                needed_properties,
                self.nstates,
                self.natoms,
    ...
    def get_coupling_property(self):
        if self.coupling == "nacs":
            return ["nacs"]
        elif self.coupling == "wf_overlap":
            return ["wf_overlap"]
    ...
    def get_ene_cou_grad(self, crd, curr_state, *, setup=False, properties=None):
        if properties is None:
            properties = []
        else:
            properties = properties.copy()
        additional_properties = {prop: None for prop in properties}
        properties += ["energy", "gradient"]
        pro = self.get_coupling_property()
        if setup is False or "nacs" in pro:
            properties += pro

        result = self.compute(properties, crd, curr_state)
        ...
        for prop in additional_properties:
            if prop in ["energy", "gradient", "nacs", "wf_overlap"]:
                continue
            additional_properties[prop] = result[prop]

        if setup is True:
            if self.coupling == "nacs":
                nac = result["nacs"]
                ene_cou_grad = namedtuple("ene_cou_grad", "ene u nac grad additional")
                return ene_cou_grad(ene, u, nac, grad, additional_properties)
            elif self.coupling == "wf_overlap":
                wf_ov = np.identity(self.nstates)
                ene_cou_grad = namedtuple("ene_cou_grad", "ene u wf_ov grad additional")
                return ene_cou_grad(ene, u, wf_ov, grad, additional_properties)
            elif self.coupling in ("non_coup", "semi_coup"):
                ene_cou_grad = namedtuple("ene_cou_grad", "ene grad additional")
                return ene_cou_grad(ene, grad, additional_properties)
    ...
    def get_hopp_u_grad(self, crd, state):
        grad = self.compute(["gradient"], crd, state.instate, same_crd=True)["gradient"]
        u = state.u
    ...
    def setup(self, state):
        ene_cou_grad = self.get_ene_cou_grad(
            state.crd, state.instate, setup=True, properties=state.save_properties
        )
        state.additional = ene_cou_grad.additional
    ...
    def new_surface(self, state, results, crd_new, t, dt):
        ene_cou_grad = self.get_ene_cou_grad(
            state.crd, state.instate, properties=state.save_properties
        )
        print("additional = ", ene_cou_grad.additional)
        state.additional = ene_cou_grad.additional
...
class PrintResults:
    ...
    def norm_coeff(self, ncoeff):
        if not isinstance(ncoeff, ndarray):
            ncoeff = array(ncoeff)
    ...
    def save_db_old(self, t, state):
        ...
        if state.method == "Surface_Hopping" and prob == "tully":
            ...
            self.db.set("currstate", state.instate)
            self.db.set("fosc", self.norm_coeff(state.ncoeff))
        elif state.method == "Surface_Hopping" and prob in ("lz", "lz_nacs"):
            if model:
                db = PySurfDB.generate_database(
                    "results.db",
            ...
            db = PySurfDB.generate_database(
                "results.db",
                data=["crd", "veloc", "energy", "time", "ekin", "epot", "etot"],
                dimensions={"natoms": natoms, "nstates": nstates},
                model=model,
            )
    ...
    def print_acknowledgment(self, state):
        ...
        ack = namedtuple(
            "ack",
            "title vel crd based actors prob coupling rescale_vel dt mdsteps instate nstates ncoeff decoherence",
        )
        return ack(
            title,
            vel,
    ...
    def print_head(self, state):
        ...
        if state.method == "Surface_Hopping":
            ...
            self.gen_results.write(
                f"Computing a trajectory surface hopping simulation:\n"
            )
            ...
            self.gen_results.write(
                f"{head.steps:>10s} {head.t:>10s} {head.ekin:>15s} {head.epot:>17s} {head.etotal:>13s}"
            ...
            self.gen_results.write(
                f"   {self.dis_dimer(ack.crd[0],ack.crd[1]):>0.4f}\n"
            )
            self.gen_results.write(f"   Initial velocity:\n")
            self.gen_results.write(
                f"   {self.dis_dimer(ack.vel[0],ack.vel[1]):>0.4f}\n"
            )
            self.gen_results.write(f"   Time step: {ack.dt}\n")
            self.gen_results.write(f"   MD steps: {ack.mdsteps}\n")
            ...
            self.gen_results.write(
                f"{head.steps:>10s} {head.t:>10s} {head.dis:>12s} {head.dis_vel:>12s}"
    ...
    def print_var(self, t, dt, sur_hop, state):
        ...
        self.gen_results.write(
            f"{var.steps:>8.0f} {var.t:>12.2f} {var.ekin:>15.3f} {var.epot:>17.4f}"
            f"{var.etotal:>13.4f} {var.hopp:>15.5f} {var.random:>11.5f} {var.state:>11.0f}\n"
        )
        if var.state != self.instate:
            self.hopping.append(
                f"Hopping from state {self.instate} to state {state.instate}"
    ...
    def print_bh_var(self, t, dt, state):
        ...
        var = var(
            int(t / dt),
            t,
            self.dis_dimer(state.crd[0], state.crd[1]),
            self.dis_dimer(state.vel[0], state.vel[1]),
            state.ekin,
            state.epot,
            state.ekin + state.epot,
            state.instate,
        ...
        self.gen_results.write(
            f"{var.steps:>8.0f} {var.t:>12.2f} {var.dis:>12.4f} {var.dis_vel:>12.4f}"
