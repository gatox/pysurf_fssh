...
class BornOppenheimer:
    ...
    def __init__(self, state):
        ...
        self.spp = SurfacePointProvider.from_questions(["energy","gradient"]+state.save_properties, self.nstates, self.natoms, config ="spp.inp", atomids = state.atomids, check_only=True)
    ...
    def setup(self, state):
        result = self.compute(['energy', 'gradient'], state.crd, curr_state=state.instate)
...
class SurfaceHopping(BornOppenheimer):
    ...
    def __init__(self, state):
        ...
        if self.coupling == "nacs" and self.prob == "tully":
            ...
            self.spp = SurfacePointProvider.from_questions(needed_properties, self.nstates, self.natoms, config ="spp.inp", atomids = state.atomids)
    ...
    def setup(self, state):
        ene_cou_grad = self.get_ene_cou_grad(state.crd, state.instate, setup=True, properties=state.save_properties)
    ...
    def new_surface(self, state, results, crd_new, t, dt):
        ene_cou_grad = self.get_ene_cou_grad(state.crd, state.instate, properties=state.save_properties)
...
class PrintResults:
    ...
    def _setup_db(self, state):
        ...
        if state.method == "Surface_Hopping" and prob == "tully":
            data=["crd","veloc","energy","time","ekin","epot","etot","populations", "fosc","currstate"]+save_properties
            if model:
                db = PySurfDB.generate_database("results.db", data=data, dimensions ={"nmodes":nmodes, "nstates":nstates}, model = model)
            else:
                db = PySurfDB.generate_database("results.db", data=data, dimensions ={"natoms":natoms, "nstates":nstates}, model = model)
        elif state.method == "Surface_Hopping" and prob in ("lz","lz_nacs"):
            data=["crd","veloc","energy","time","ekin","epot","etot","currstate"]+save_properties
            if model:
                db = PySurfDB.generate_database("results.db", data=data, dimensions ={"nmodes":nmodes, "nstates":nstates}, model = model)
            else:
                db = PySurfDB.generate_database("results.db", data=data, dimensions ={"natoms":natoms, "nstates":nstates}, model = model)
        elif state.method == "Born_Oppenheimer":
            data = ["crd","veloc","energy","time","ekin","epot","etot"] + save_properties
            db = PySurfDB.generate_database("results.db", data=data, dimensions ={"natoms":natoms, "nstates":nstates}, model = model)
        else:
