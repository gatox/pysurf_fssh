...
class CoupledTrajectories:
    ...
    def run_coupled(self, run_trajectory):
        shared_data = multiprocessing.Array("d", self.n_traj)  # Shared memory
        with multiprocessing.Pool(processes=self.n_traj) as pool:
            pool.starmap(run_trajectory, [(i, shared_data) for i in range(self.n_traj)])

        # After all trajectories are complete, process the shared data
        self.process_shared_data(shared_data)
