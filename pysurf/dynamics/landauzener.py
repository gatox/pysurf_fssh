...
class LandauZener(PropagatorBase):
    ...
    def _run(self, nsteps, dt, seed=16661):
        ...
        for istep in range(self.start, nsteps + 1):
            ...
            if self.iselected is not None:
                # change active state
                self.dE = (
                    self.data["energy"][self.iselected]
                    - self.data["energy"][self.iactive]
                )
                ...
                if self.dE > self.ekin:
                    ...
                    self.logger.info(
                        f"*   LandauZener hop: {self.iactive} -> {self.iselected}\n"
                    )
                    ...
                    self.a = get_acceleration(
                        self.data["gradient"][self.iactive], self.masses
                    )
    ...
    def call_spp(self, crd=None, gradstate=None, same_crd=False):
        ...
        res = self.spp.request(
            crd, self.properties, states=[gradstate], same_crd=same_crd
        )
        return res
    ...
    def setup_new(self):
        ...
        if self.iactive > self.nstates - 1:
            self.logger.error(
                "Initial state is higher than number of states of the calculation"
            )
        # set starting crds
    ...
    def lz_select(self):
        ...
        for istate in range(self.nstates):
            ...
            if abs_gt(d_curr, d_prev_step) and abs_gt(d_two_step_prev, d_prev_step):
                curr_hop_prob = self._lz_probability(
                    self.dt, d_curr, d_prev_step, d_two_step_prev
                )
    ...
    def _lz_probability(self, dt, d_curr, d_prev_step, d_two_step_prev):
        ...
        return np.exp(
            (-np.pi / 2.0)
            * np.sqrt(abs(d_prev_step) ** 3 / abs(finite_difference_grad))
        )
