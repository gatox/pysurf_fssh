...
class AnalyseFitPesNm(Colt):
    ...
    _plot_pes = {"yes": "plot_inputfile = plot_fit_pes_nm.inp :: file", "no": ""}
    ...
    def __init__(self, config, plot_config):
        ...
        if config["plot_pes"] == "yes":
            ...
            myax = myplt.line_plot(
                ...
                y_units_in=("energy", config["energy_units"]),
                ax=None,
                show_plot=False,
                save_plot=False,
            )
    ...
    def save_pes(self, filename, database):
        ...
        with open(filename, "w") as f:
            f.write(
                "\n".join(
                    f"{i} {str_join(fitted)} {str_join(exact)}"
