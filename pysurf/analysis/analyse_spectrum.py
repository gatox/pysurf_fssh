...
class Broadening(Colt):
    ...
    def broadening(self, data):
        ...
        res[:, 0] = np.linspace(
            self.config["energy_start"],
            self.config["energy_end"],
    ...
    def g_vec(fosc, e0, w):
        return np.vecotrize(
            lambda e: fosc * np.exp(-np.ln(2) * ((e - e0) / w * 2) ** 2)
        )
...
class AnalyseSpectrum(Colt):
    ...
    _save_data = {"yes": "data_file = spectrum/spectrum.dat :: file", "no": " "}

    _broadening = {"yes": Broadening, "no": NoFurtherQuestions}
    ...
    def _extend_user_input(cls, questions):
        questions.generate_cases(
            "save_data", {name: mode for name, mode in cls._save_data.items()}
        )
        questions.generate_cases(
            "broadening",
            {name: mode.questions for name, mode in cls._broadening.items()},
        )
        questions.generate_cases(
            "plot_spectrum", {name: mode for name, mode in cls._plot_spectrum.items()}
        )
    ...
    def from_inputfile(cls, inputfile):
        ...
        if config["plot_spectrum"].value == "yes":
            ...
            if exists_and_isfile(plot_input):
                plot_config = Plot.generate_input(
                    plot_input, config=plot_input, presets=presets
                )
            else:
                plot_config = Plot.generate_input(
                    plot_input, config=plot_config, presets=presets
                )
    ...
    def __init__(self, config, plot_config=None):
        ...
        data = []
        for energy, fosc in zip(
            np.copy(sampling._db["energy"]), np.copy(sampling._db["fosc"])
        ):
        ...
        Plot(plot_config).line_plot(
            cont_data,
            ("energy", self.config["energy_units"]),
            y_units_in=None,
            ax=None,
            line_props={"linestyle": "solid"},
        )
