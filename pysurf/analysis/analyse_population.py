...
class AnalysePopulation(Colt):
    ...
    _save_data = {"yes": "data_file = population.dat :: file", "no": " "}

    _plot_population = {"yes": "plot_inputfile = plot_population.inp :: file", "no": ""}
    ...
    def _extend_user_input(cls, questions):
        ...
        questions.generate_cases(
            "plot_population",
            {name: mode for name, mode in cls._plot_population.items()},
        )
    ...
    def __init__(self, config, plot_config=None):
        ...
        for idx, propfile in enumerate(propfiles):
            ...
            if (
                np.max(
                    np.abs(
                        data[: min(time_steps, len(dbtime)), 0]
                        - dbtime[: min(time_steps, len(dbtime))]
                    )
                )
                < 0.1
            ):
                if len(dbtime) > time_steps and times_missing is True:
                    data[time_steps : len(dbtime), 0] = dbtime[
                        time_steps : min(nsteps, len(dbtime))
                    ]
