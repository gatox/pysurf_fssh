...
class PESModel(Colt):
    ...
    def _extend_user_input(cls, questions):
        questions.generate_cases(
            "model",
            {name: model.questions for name, model in ModelBase._models.items()},
        )
    ...
    def __init__(self, config):
        ...
        for idx, point in enumerate(data[:, 0]):
            ...
            data[idx, 1:] = model.get(
                Request(crd, properties=["energy"], states=[i for i in range(nstates)])
            )["energy"]
        print(data)
        plot_config = {}
        ...
        if exists_and_isfile(self._plot_input):
            plot_config = Plot.generate_input(
                self._plot_input, config=self._plot_input, presets=presets
            )
        else:
            plot_config = Plot.generate_input(
                self._plot_input, config=None, presets=None
            )
