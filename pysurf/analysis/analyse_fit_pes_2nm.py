...
class AnalyseFitPes2Nm(Colt):
    ...
    _save_data = {"yes": "data_file = fit_pes_2nm.dat :: file", "no": " "}

    _plot_pes = {"yes": "plot_inputfile = plot_fit_pes_2nm.inp :: file", "no": ""}
    ...
    def _extend_user_input(cls, questions):
        ...
        questions.generate_cases(
            "plot_pes", {name: mode for name, mode in cls._plot_pes.items()}
        )
    ...
    def from_inputfile(cls, inputfile):
        ...
        if config["plot_pes"].value == "yes":
            ...
            if exists_and_isfile(plot_input):
                plot_config = Plot3D.generate_input(
    ...
    def __init__(self, config, plot_config):
        ...
        self.spp = SurfacePointProvider.from_config(
            config_spp,
            properties,
            self.nstates,
            natoms,
    ...
    def _get_spp_config(self, filename):
        questions = SurfacePointProvider.generate_user_input(
            presets="""
                use_db=yes :: yes
                [use_db(yes)]
                fit_only = yes :: yes
    ...
    def generate_crds(self, moldenfile, mode, start=-5, end=5, npoints=50):
        ...
        qx, qy = np.mgrid[
            start[0] : end[0] : npoints[0] * 1j, start[1] : end[1] : npoints[1] * 1j
        ]
        i = 0
        for qxi, qyi in zip(np.ravel(qx), np.ravel(qy)):
            crds[i] = (
                crd
                + qxi * modes[mode[0]].displacements
                + qyi * modes[mode[1]].displacements
            )
            i += 1
        return qx, qy, crds
