...
class PyrazineSala(Model):
    ...
    E = {0: 0.0, 1: 3.93, 2: 4.45, 3: 4.79, 4: 5.38}

    # Kappa values [eV] from Table 4
    k = {
        "v6a": {1: -0.081, 2: -0.168, 3: 0.128, 4: -0.184},
    ...
    g = {
        "v10a": {1: -0.012, 2: -0.048, 3: -0.012, 4: -0.013},
    ...
    def __init__(self, config):
        ...
        self.modes = [
            Mode(freq, dis) for freq, dis in zip(self.frequencies, self.displacements)
        ]
    ...
    def diab_en(self, crd):
        ...
        for state_idx, state in enumerate(self.states):
            ...
            for idx, mode in enumerate(self.q):
                ...
                diab_en[state_idx, state_idx] += (
                    self.conv_cm(self.w[mode]) / 2.0 * crd[idx] ** 2
                )

                # Add kappa terms
                if mode in self.k and state != 0:
                    diab_en[state_idx, state_idx] += (
                        self.conv_ev(self.k[mode][state]) * crd[idx]
                    )

                # Add gamma terms
                if mode in self.g and state != 0:
                    diab_en[state_idx, state_idx] += (
                        self.conv_ev(self.g[mode][state]) * crd[idx] ** 2
                    )
