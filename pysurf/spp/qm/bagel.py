...
class Bagel(AbinitioBase):
    ...
    active_space_settings = {"active": [1, 2]}
    ...
    def _update_settings(self, config):
        ...
        self.xms_caspt2_settings = {
            key: value for key, value in config["caspt2"].items()
        }
        self.active_space_settings = {
            key: value for key, value in config["active_space"].items()
        }
    ...
    def _read_nacs(self):
        nacs = {}
        prefixed = [
            filename for filename in os.listdir(".") if filename.startswith("NACME")
        ]
    ...
    def _read_grads(self, state):
        ...
        prefixed = [
            filename for filename in os.listdir(".") if filename.startswith("FORCE")
        ]
        output = prefixed[0]
    ...
    def _do_sa_casscf_ene_grad_nacs(self, state):
        ...
        if self.active == "yes":
            casscf_settings = UpdatableDict(
                self.casscf_settings, self.active_space_settings
            )
        else:
        ...
        if self.caspt2 == "yes":
            ...
            self._write_caspt2_input(
                self.filename,
                settings.items(),
                casscf_settings.items(),
                xms_caspt2_settings.items(),
                root,
            )
        elif self.caspt2 == "not":
            self._write_input(
                self.filename, settings.items(), casscf_settings.items(), root
            )
    ...
    def _nacs_no_last(self, nac_i, nac_j):
        return '{ "title" : "nacme", "target" : %(i)s, "target2" : %(j)s },' % {
            "i": nac_i,
            "j": nac_j,
        }
    ...
    def _write_input(self, filename, remsection, cassection, gradsection):
        ...
        with open(filename, "w") as f:
            if self.f_osc == "yes":
                ...
                f.write(
                    self.bagel_grad_ini.render(
                        state=self._grad_yes_no_coupling(gradsection)
                    )
                )
                if self.couplings == "yes":
                    for i in range(self.nstates):
                        for j in range(self.nstates):
                            if j > i:
                                if i == int(self.nstates - 2) and j == int(
                                    self.nstates - 1
                                ):
                                    f.write(
                                        self.bagel_nacs.render(
                                            nacs=self._nacs_last(i, j)
                                        )
                                    )
                                else:
                                    f.write(
                                        self.dc.render(nacs=self._nacs_no_last(i, j))
                                    )
    ...
    def _write_caspt2_input(
        self, filename, remsection, cassection, caspt2section, gradsection
    ):
        """write input file for BAGEL"""
        with open(filename, "w") as f:
            if self.f_osc == "yes":
                ...
                f.write(self.bagel_osc_ini.render())
                f.write(
                    self.bagel_grad_end_caspt2.render(
                        caspt2=caspt2section, casscf=cassection
                    )
                )
                f.write(self.bagel_end_wo_coma.render())
            else:
...
if __name__ == "__main__":
    ...
    spp = SurfacePointProvider.from_questions(
        ["energy", "fosc"], nstates, natoms, atomids=atomids, config="test.inp"
    )
