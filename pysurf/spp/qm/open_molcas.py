...
Gradient_OpMol = Event(
    "Gradient",
    "xgrep",
    {
        "keyword": "Molecular gradients",
        "ilen": length,
        "ishift": 8,
    },
    func=get_gradient,
...
NACs_Index_OpMol = Event(
    "NACs_Index",
    "xgrep",
    {
        "keyword": "Lagrangian multipliers are calculated for states no.",
        "ilen": 1,
        "ishift": 0,
    },
...
NACs_OpMol = Event(
    ...
    "xgrep",
    {
        "keyword": "Total derivative coupling",
        "ilen": length,
        "ishift": 8,
    },
    settings={"multi": True, "reset": True},
...
Fosc_OpMol = Event(
    ...
    "xgrep",
    {
        "keyword": "Dipole transition strengths",
        "ilen": length_osc,
        "ishift": 6,
    },
    settings={"multi": True, "reset": True},
...
class OpenMolcas(AbinitioBase):
    ...
    def _update_settings(self, config):
        ...
        self.rasscf_settings = {
            key: self.no_square_brakets(config[key]) for key in self.rasscf_settings
        }
        self.xms_caspt2_settings = {
            key: self.no_square_brakets(value)
            for key, value in config["caspt2"].items()
        }
    ...
    def _read_f_osc(self, output):
        ...
        if not isinstance(out["Fosc_OpMol"][0], list):
            out_f_osc = [0.0] + [out["Fosc_OpMol"][0]]
        else:
            out_f_osc = [0.0] + [value for value in out["Fosc_OpMol"][0]]
        return out_f_osc[: self.nstates]  # just interactions with the groud states

    def _read_nacs(self, output):
        out = self.reader(
            output, ["NACs_Index_OpMol", "NACs_OpMol"], {"natoms": self.molecule.natoms}
        )
        ...
        if self.nstates > 2:
            for i in range(leng):
                nacs.update(
                    {
                        (
                            int(out["NACs_Index_OpMol"][i][0] - 1),
                            int(out["NACs_Index_OpMol"][i][1] - 1),
                        ): np.array(nac[i])
                    }
                )
                nacs.update(
            ...
            nacs.update(
                {
                    (
                        int(out["NACs_Index_OpMol"][0][0] - 1),
                        int(out["NACs_Index_OpMol"][0][1] - 1),
                    ): np.array(nac[0])
                }
            )
            nacs.update(
    ...
    def _do_sa_casscf_ene_grad_nacs(self, state):
        ...
        if self.caspt2 == "yes":
            ...
            self._write_caspt2_input(
                self.filename,
                settings.items(),
                rasscf_settings.items(),
                xms_caspt2_settings.items(),
                root,
            )
        elif self.caspt2 == "not":
            self._write_input(
                self.filename, settings.items(), rasscf_settings.items(), root
            )
    ...
    def _write_input(self, filename, remsection, rassection, gradsection):
        ...
        with open(filename, "w") as f:
            if self.f_osc == "yes":
                f.write(
                    self.tpl.render(
                        natoms=self.natoms, mol=self.molecule, remsection=remsection
                    )
                )
    ...
    def _write_caspt2_input(
        self, filename, remsection, rassection, caspt2section, gradsection
    ):
        """write input file for open_molcas"""
        with open(filename, "w") as f:
            if self.f_osc == "yes":
                    ...
                    for i in range(self.nstates):
                        for j in range(self.nstates):
                            if j > i:
                                nac = [i + 1, j + 1]
                                f.write(
                                    self.dc_caspt2.render(
                                        caspt2=caspt2section,
                                        nac=self.no_square_brakets(nac),
                                    )
                                )
            f.write(self.end_do.render())
