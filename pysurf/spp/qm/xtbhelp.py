...
def read_xtb_gradient(iterator, NAtoms=0):
    ...
    out = (
        np.array(
            list(
                list(map(float, next(iterator).replace("D", "E").split()))
...
def read_xtb_hessian(iterator, NAtoms=0):
    ...
    return (
        np.array(list(list(map(float, line.split())) for line in iterator)).flatten(),
        1,
    )
...
Dipole = Event(
    "Dipole",
    "grep",
    {"keyword": "molecular dipole:", "ilen": 1, "ishift": 3},
    func="split",
    func_kwargs={"idx": [1, 2, 3], "typ": [float, float, float]},
)
#
Energy = Event(
    "Energy",
    "grep",
    {"keyword": "TOTAL ENERGY", "ilen": 1, "ishift": 0},
    func="split",
