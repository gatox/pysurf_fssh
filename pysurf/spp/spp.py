...
class AbinitioFactory(Plugin):
    ...
    def _extend_user_input(cls, questions):
        questions.add_branching(
            "software",
            {name: software.colt_user_input for name, software in cls.software.items()},
        )
...
class ModelFactory(Plugin):
    ...
    def _extend_user_input(cls, questions):
        questions.generate_cases(
            "model",
            {name: model.colt_user_input for name, model in cls._models.items()},
        )
...
class SurfacePointProvider(Colt):
    ...
    # different modes
    _modes = {
        "ab-initio": AbinitioFactory,
        "model": ModelFactory,
    }
    # use_db
    _database = {"yes": DataBaseInterpolation, "no": NoFurtherQuestions}
    ...
    def _extend_user_input(cls, questions):
        ...
        questions.generate_cases(
            "mode", {name: mode.colt_user_input for name, mode in cls._modes.items()}
        )
        questions.generate_cases(
            "use_db",
            {name: mode.colt_user_input for name, mode in cls._database.items()},
        )
    ...
    def __init__(
        ...
        *,
        ...
        if logger is None:
            ...
            self.logger = logger
        #
        self._request, self._interface = self._select_interface(
            mode_config, use_db, properties, natoms, nstates, nghost_states, atomids
        )
    ...
    def request(self, crd, properties, states=None, same_crd=False):
        ...
        """
        datacontainer = self._request.request(
            crd, properties, states, same_crd=same_crd
        )
        return self._interface.get(datacontainer)

    def _select_interface(
        self, mode_config, use_db, properties, natoms, nstates, nghost_states, atomids
    ):
        ...
        # If an ab initio calculation is used and if a database is used
            ...
            interface = AbinitioFactory.plugin_from_config(
                mode_config["software"], atomids, nstates, nghost_states
            )
        else:
        ...
        if use_db == "yes":
            ...
            interface = DataBaseInterpolation.from_config(
                use_db,
                interface,
                natoms,
                nstates,
    ...
    def _check_properties(self, properties, interface):
        ...
        if any(prop not in interface.implemented for prop in properties):
            self.logger.error(
                f"""The Interface does not provide all properties:
Needed: {properties}
...
def get_spp(
    ...
    *,
    ...
    """
    return SurfacePointProvider.from_questions(
        properties,
        nstates,
        natoms,
        nghost_states=nghost_states,
